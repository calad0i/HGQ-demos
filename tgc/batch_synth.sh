...
function synth {
    ...
    perl -pi -e "s/#pragma HLS (?:PIPELINE|DATAFLOW)/#pragma HLS pipeline/" firmware/*.cpp
    # remove clock uncertainty to match previous work, synths w/o timing convergence are removed from results
    perl -pi -e "s/set clock_uncertainty .*%/set clock_uncertainty 0%/" project.tcl

    vitis-run --mode hls --tcl build_prj.tcl > ./synth.log
